...
function [Y_downstream,OR_outflow] = ds_capital_nonlinear_1o_simulation(inflows,C,OR,time_step,N)
n = length(inflows);
...
save('Simulation/ds_capital_nonlinear_1o_simulation/ds_capital_nonlinear_1o_simulation_data.mat');

% Simulate System
simu_file = 'ds_capital_nonlinear_1o_simulation_simu';
simIn = Simulink.SimulationInput(simu_file);
simIn = loadVariablesFromMATFile(simIn,"Simulation/ds_capital_nonlinear_1o_simulation/ds_capital_nonlinear_1o_simulation_data.mat");
warning('off');
out = sim(simIn);
clc;
