...
%%% 'linear' means the feedback function is linear
...
function [Y_downstream,OR_outflow] = ds_capital_linear_1o_simulation_notint(inflows,C,K_constant,YX,Q0or,time_step,N)
n = length(inflows);
...
save('Simulation/ds_capital_linear_1o_simulation_notint/ds_capital_linear_1o_simulation_notint_data.mat');

% Simulate System
simu_file = 'ds_capital_linear_1o_simulation_notint_simu';
simIn = Simulink.SimulationInput(simu_file);
simIn = loadVariablesFromMATFile(simIn,"Simulation/ds_capital_linear_1o_simulation_notint/ds_capital_linear_1o_simulation_notint_data.mat");
warning('off');
out = sim(simIn);
clc;
